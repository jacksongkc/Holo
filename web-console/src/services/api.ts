...
function applyAuthHeaders(headers: Record<string, string>) {
  ...
  if (user) {
    if (user.role) {
      headers["X-Holo-Role"] = user.role;
    }
    if (user.userId) {
      headers["X-Holo-UserID"] = user.userId;
    }
    if (user.username) {
      headers["X-Holo-Username"] = user.username;
    }
  }
}
...
export const api = {
  auth: {
    login: (body: { username: string; password: string; twoFactorCode?: string }) =>
      request<LoginResponse>("/v1/auth/login", { method: "POST", body }),
  },
  users: {
    ...
    get: (userId: string) => request<User>(`/v1/users/${encodeURIComponent(userId)}`),
    me: () => request<User>("/v1/users/me"),
    update: (userId: string, body: { username?: string; email?: string; password?: string; role?: string; status?: string }) =>
      request<User>(`/v1/users/${encodeURIComponent(userId)}`, { method: "PUT", body }),
    delete: (userId: string) => request<void>(`/v1/users/${encodeURIComponent(userId)}`, { method: "DELETE" }),
    changePassword: (userId: string, body: { oldPassword: string; newPassword: string }) =>
      request<{ message: string }>(`/v1/users/change-password/${encodeURIComponent(userId)}`, { method: "POST", body }),
    twoFactor: {
      getSetup: () => request<{ secret: string; qrCodeUrl: string; enabled: boolean; issuer: string; accountName: string }>("/v1/users/two-factor"),
      enable: (body: { secret: string; code: string }) =>
        request<{ enabled: boolean }>("/v1/users/two-factor", { method: "POST", body }),
      disable: () =>
        request<{ enabled: boolean }>("/v1/users/two-factor", { method: "DELETE" }),
    },
  },
  ops: {
    health: async () => {
    ...
    getSettings: () => request<SystemSettings>("/v1/system/settings"),
    saveSettings: (settings: SystemSettings) =>
      request<SystemSettings>("/v1/system/settings", { method: "PUT", body: settings }),
